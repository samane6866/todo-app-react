...
function EditTask({ task, onSave, onCancel }) {
  // State to manage the edited task title,it checks if the "task" has truthy value and its'not null,etc.. in case that is true task,it use task.title as initial value for editedTask state.
      ...
      <form>
        ...
        <div className="d-flex justify-content-center my-4">
          <button
            ...
            className="btn btn-info mx-2 text-white fw-bold"
          >
            Save
          </button>
          <button
            type="submit"
            onClick={onCancel}
            className="btn btn-secondary mx-2 text-white fw-bold"
          >
            {onCancel}Cancel
          </button>
        </div>
